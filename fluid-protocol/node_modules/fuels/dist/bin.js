...
// src/index.ts
__reExport(src_exports, require("@fuel-ts/abi-coder"));
__reExport(src_exports, require("@fuel-ts/address"));
__reExport(src_exports, require("@fuel-ts/address/configs"));
__reExport(src_exports, require("@fuel-ts/contract"));
__reExport(src_exports, require("@fuel-ts/crypto"));
__reExport(src_exports, require("@fuel-ts/errors"));
__reExport(src_exports, require("@fuel-ts/hasher"));
__reExport(src_exports, require("@fuel-ts/interfaces"));
__reExport(src_exports, require("@fuel-ts/math"));
__reExport(src_exports, require("@fuel-ts/math/configs"));
__reExport(src_exports, require("@fuel-ts/program"));
__reExport(src_exports, require("@fuel-ts/transactions"));
__reExport(src_exports, require("@fuel-ts/utils"));
__reExport(src_exports, require("@fuel-ts/account"));
__reExport(src_exports, require("@fuel-ts/transactions/configs"));
__reExport(src_exports, require("@fuel-ts/account/configs"));
...
async function deployContract(wallet, binaryPath, abiPath, storageSlotsPath, deployConfig, contractPath, tomlContents) {
  ...
  if ((0, import_fs3.existsSync)(storageSlotsPath)) {
    const storageSlots3 = JSON.parse((0, import_fs3.readFileSync)(storageSlotsPath, "utf-8"));
...
var import_contract2 = require("@fuel-ts/contract");
...
async function deployPredicate(wallet, binaryPath, abiPath) {
  (0, import_console.debug)(`Deploying predicate for ABI: ${abiPath}`);
  const bytecode2 = (0, import_fs4.readFileSync)(binaryPath);
  const abi2 = JSON.parse((0, import_fs4.readFileSync)(abiPath, "utf-8"));
  const factory = new import_contract2.ContractFactory(bytecode2, abi2, wallet);
...
async function deployPredicates(config) {
  ...
  for (let i = 0; i < predicatesLen; i++) {
    ...
    let abi2 = JSON.parse((0, import_fs4.readFileSync)(abiPath, "utf-8"));
    ...
    predicates.push({
      path: predicatePath,
      predicateRoot,
      loaderBytecode: (0, import_utils.arrayify)(loaderBytecode),
      abi: abi2
    });
  }
  return predicates;
...
var import_contract3 = require("@fuel-ts/contract");
...
async function deployScript(wallet, binaryPath, abiPath) {
  (0, import_console2.debug)(`Deploying script for ABI: ${abiPath}`);
  const bytecode2 = (0, import_fs5.readFileSync)(binaryPath);
  const abi2 = JSON.parse((0, import_fs5.readFileSync)(abiPath, "utf-8"));
  const factory = new import_contract3.ContractFactory(bytecode2, abi2, wallet);
...
async function deployScripts(config) {
  ...
  for (let i = 0; i < scriptsLen; i++) {
    ...
    let abi2 = JSON.parse((0, import_fs5.readFileSync)(abiPath, "utf-8"));
    if (configurableOffsetDiff) {
      abi2 = adjustOffsets(abi2, configurableOffsetDiff);
    }
    (0, import_console2.debug)(`Script deployed: ${projectName} - ${blobId}`);
    scripts.push({
      path: scriptPath,
      loaderBytecode: (0, import_utils2.arrayify)(loaderBytecode),
      abi: abi2
    });
  }
  return scripts;
...
function savePredicateFiles(predicates, _config) {
  for (const { path, predicateRoot, loaderBytecode, abi: abi2 } of predicates) {
    ...
    (0, import_fs6.writeFileSync)(abiPath, JSON.stringify(abi2, null, 2));
  }
}
...
function saveScriptFiles(scripts, _config) {
  for (const { path, loaderBytecode, abi: abi2 } of scripts) {
    ...
    (0, import_fs7.writeFileSync)(abiPath, JSON.stringify(abi2, null, 2));
  }
}
...
var autoStartFuelCore = async (config) => {
  ...
  if (config.autoStartFuelCore) {
    ...
    config.privateKey = import_utils3.defaultConsensusKey;
  }
  return fuelCore;
};
...
async function loadConfig(cwd) {
  ...
  const config = {
    ...
    privateKey: import_utils4.defaultConsensusKey,
    ...userConfig,
    basePath: cwd,
    forcPath,
    fuelCorePath,
...
function withConfig(program, command, fn) {
  return async () => {
    ...
    try {
      ...
      log(`\u{1F389}  ${(0, import_utils5.capitalizeString)(command)} completed successfully!`);
    } catch (err) {
