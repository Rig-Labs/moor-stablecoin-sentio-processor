...
export function generateBoundFunctionCallEncoder(fn, includeArgTypes) {
  ...
  ${declName ?? fn.name}(${generateInputTypes(fn.inputs, {
    ...
    return this.view.encodeCall.${declName}(${fn.inputs.length > 0 ? fn.inputs.map((input, index) => input.name || `arg${index}`).join(',') + ',' : ''}{chainId: this.context.chainId.toString(), address: this.context.address, blockTag: blockTagWithOverride})
  }
  `
    ];
}
