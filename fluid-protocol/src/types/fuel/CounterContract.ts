/* Autogenerated file. Do not edit manually. */

/* eslint-disable max-classes-per-file */
/* eslint-disable @typescript-eslint/no-unused-vars */
/* eslint-disable @typescript-eslint/consistent-type-imports */

/*
  Fuels version: 0.94.9
*/

import { Contract, Interface } from "fuels";
import type {
  Provider,
  Account,
  StorageSlot,
  AbstractAddress,
  BigNumberish,
  BN,
  FunctionFragment,
  InvokeFunction,
} from 'fuels';

import type { Option, Enum, Result } from './common.js';

export enum ColorErrorInput { Blue = 'Blue', Green = 'Green', Red = 'Red', Other = 'Other' };
export enum ColorErrorOutput { Blue = 'Blue', Green = 'Green', Red = 'Red', Other = 'Other' };

export type FooInput = { bar: BigNumberish, baz: boolean };
export type FooOutput = { bar: BN, baz: boolean };

const abi = {
  "programType": "contract",
  "specVersion": "1",
  "encodingVersion": "1",
  "concreteTypes": [
    {
      "type": "b256",
      "concreteTypeId": "7c5ee1cecf5f8eacd1284feb5f0bf2bdea533a51e2f0c9aabe9236d335989f3b"
    },
    {
      "type": "enum ColorError",
      "concreteTypeId": "3c5893a10e00114df7da4af22e21bc4e6abb442f0a9a0236892144ef8b5ee356",
      "metadataTypeId": 2
    },
    {
      "type": "enum std::option::Option<str[1]>",
      "concreteTypeId": "41a3629cd05378f7fb5c2027250fdfb884768575efbdad2994495e18752a1b96",
      "metadataTypeId": 3,
      "typeArguments": [
        "6354581bfa62ac188c9f243cc30e1bf38a150773f871be8765351ef8c5db6c1e"
      ]
    },
    {
      "type": "enum std::result::Result<struct Foo,enum ColorError>",
      "concreteTypeId": "19bd183cd8661e37253fee96c5294d94f25d166c405d72794bf29e48d881f69d",
      "metadataTypeId": 4,
      "typeArguments": [
        "7cfc11f7909eb46c488c03c985d20c9937ae8b9ed2083fa0007b3da8f1a79d87",
        "3c5893a10e00114df7da4af22e21bc4e6abb442f0a9a0236892144ef8b5ee356"
      ]
    },
    {
      "type": "str[1]",
      "concreteTypeId": "6354581bfa62ac188c9f243cc30e1bf38a150773f871be8765351ef8c5db6c1e"
    },
    {
      "type": "str[25]",
      "concreteTypeId": "cb0ee391dbed30daa3cdad5577605a178185248dc00dcf53960b10eea0cab237"
    },
    {
      "type": "struct Foo",
      "concreteTypeId": "7cfc11f7909eb46c488c03c985d20c9937ae8b9ed2083fa0007b3da8f1a79d87",
      "metadataTypeId": 7
    },
    {
      "type": "u32",
      "concreteTypeId": "d7649d428b9ff33d188ecbf38a7e4d8fd167fa01b2e10fe9a8f9308e52f1d7cc"
    },
    {
      "type": "u64",
      "concreteTypeId": "1506e6f44c1d6291cdf46395a8e573276a4fa79e8ace3fc891e092ef32d1b0a0"
    }
  ],
  "metadataTypes": [
    {
      "type": "()",
      "metadataTypeId": 0
    },
    {
      "type": "bool",
      "metadataTypeId": 1
    },
    {
      "type": "enum ColorError",
      "metadataTypeId": 2,
      "components": [
        {
          "name": "Blue",
          "typeId": 0
        },
        {
          "name": "Green",
          "typeId": 0
        },
        {
          "name": "Red",
          "typeId": 0
        },
        {
          "name": "Other",
          "typeId": 0
        }
      ]
    },
    {
      "type": "enum std::option::Option",
      "metadataTypeId": 3,
      "components": [
        {
          "name": "None",
          "typeId": 0
        },
        {
          "name": "Some",
          "typeId": 6
        }
      ],
      "typeParameters": [
        6
      ]
    },
    {
      "type": "enum std::result::Result",
      "metadataTypeId": 4,
      "components": [
        {
          "name": "Ok",
          "typeId": 6
        },
        {
          "name": "Err",
          "typeId": 5
        }
      ],
      "typeParameters": [
        6,
        5
      ]
    },
    {
      "type": "generic E",
      "metadataTypeId": 5
    },
    {
      "type": "generic T",
      "metadataTypeId": 6
    },
    {
      "type": "struct Foo",
      "metadataTypeId": 7,
      "components": [
        {
          "name": "bar",
          "typeId": "1506e6f44c1d6291cdf46395a8e573276a4fa79e8ace3fc891e092ef32d1b0a0"
        },
        {
          "name": "baz",
          "typeId": 1
        }
      ]
    }
  ],
  "functions": [
    {
      "inputs": [
        {
          "name": "foo",
          "concreteTypeId": "7cfc11f7909eb46c488c03c985d20c9937ae8b9ed2083fa0007b3da8f1a79d87"
        },
        {
          "name": "option",
          "concreteTypeId": "41a3629cd05378f7fb5c2027250fdfb884768575efbdad2994495e18752a1b96"
        }
      ],
      "name": "complex",
      "output": "19bd183cd8661e37253fee96c5294d94f25d166c405d72794bf29e48d881f69d",
      "attributes": [
        {
          "name": "storage",
          "arguments": [
            "read",
            "write"
          ]
        }
      ]
    },
    {
      "inputs": [],
      "name": "count",
      "output": "1506e6f44c1d6291cdf46395a8e573276a4fa79e8ace3fc891e092ef32d1b0a0",
      "attributes": [
        {
          "name": "storage",
          "arguments": [
            "read"
          ]
        }
      ]
    },
    {
      "inputs": [
        {
          "name": "add",
          "concreteTypeId": "d7649d428b9ff33d188ecbf38a7e4d8fd167fa01b2e10fe9a8f9308e52f1d7cc"
        },
        {
          "name": "two",
          "concreteTypeId": "1506e6f44c1d6291cdf46395a8e573276a4fa79e8ace3fc891e092ef32d1b0a0"
        },
        {
          "name": "three",
          "concreteTypeId": "7c5ee1cecf5f8eacd1284feb5f0bf2bdea533a51e2f0c9aabe9236d335989f3b"
        }
      ],
      "name": "increment",
      "output": "1506e6f44c1d6291cdf46395a8e573276a4fa79e8ace3fc891e092ef32d1b0a0",
      "attributes": [
        {
          "name": "storage",
          "arguments": [
            "read",
            "write"
          ]
        }
      ]
    }
  ],
  "loggedTypes": [
    {
      "logId": "1515152261580153489",
      "concreteTypeId": "1506e6f44c1d6291cdf46395a8e573276a4fa79e8ace3fc891e092ef32d1b0a0"
    },
    {
      "logId": "9006093109815063660",
      "concreteTypeId": "7cfc11f7909eb46c488c03c985d20c9937ae8b9ed2083fa0007b3da8f1a79d87"
    },
    {
      "logId": "14631882454972838106",
      "concreteTypeId": "cb0ee391dbed30daa3cdad5577605a178185248dc00dcf53960b10eea0cab237"
    },
    {
      "logId": "15520703124961489725",
      "concreteTypeId": "d7649d428b9ff33d188ecbf38a7e4d8fd167fa01b2e10fe9a8f9308e52f1d7cc"
    },
    {
      "logId": "8961848586872524460",
      "concreteTypeId": "7c5ee1cecf5f8eacd1284feb5f0bf2bdea533a51e2f0c9aabe9236d335989f3b"
    }
  ],
  "messagesTypes": [],
  "configurables": []
};

const storageSlots: StorageSlot[] = [];

export class CounterContractInterface extends Interface {
  constructor() {
    super(abi);
  }

  declare functions: {
    complex: FunctionFragment;
    count: FunctionFragment;
    increment: FunctionFragment;
  };
}

export class CounterContract extends Contract {
  static readonly abi = abi;
  static readonly storageSlots = storageSlots;

  declare interface: CounterContractInterface;
  declare functions: {
    complex: InvokeFunction<[foo: FooInput, option?: Option<string>], Result<FooOutput, ColorErrorOutput>>;
    count: InvokeFunction<[], BN>;
    increment: InvokeFunction<[add: BigNumberish, two: BigNumberish, three: string], BN>;
  };

  constructor(
    id: string | AbstractAddress,
    accountOrProvider: Account | Provider,
  ) {
    super(id, abi, accountOrProvider);
  }
}
